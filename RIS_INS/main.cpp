...
int main()
{
	...
	if (typeCorr != WITHOUTCORR)
	{
		...
		std::cout << "выводить ли в течение переходного процесса вычисления по базовой платформе? Да-1/Нет-0" << std::endl;
	...
	// РАБОТА НАВ АЛГОРИТМА И ЗАПИСЬ РЕЗУЛЬТАТОВ МОДЕЛИРОВАНИЯ В ФАЙЛ
